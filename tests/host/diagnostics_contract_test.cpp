...
int main() {
	...
	const auto disabled = Strata::allocationDiagnostics();
	assert(disabled.attempts == 0);
	assert(disabled.successes == 0);
	assert(disabled.failures == 0);
	assert(disabled.invalidRequests == 0);
	assert(disabled.preferredExternalFallbacks == 0);
	assert(disabled.requestedBytes == 0);
	assert(disabled.successfulBytes == 0);
	assert(disabled.failedBytes == 0);
	...
	assert(!internal.usedBytes.has_value());
	assert(!internal.peakUsedBytes.has_value());
	...
	assert(!external.usedBytes.has_value());
	assert(!external.peakUsedBytes.has_value());
