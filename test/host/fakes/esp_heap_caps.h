...
inline bool fake_heap_caps_should_fail(std::uint32_t caps) {
    ...
    if (fake_heap_caps_is_external(caps)) {
        ...
        return fake_heap_caps_fail_external || fake_heap_caps_external_total == 0;
    }
...
inline void *heap_caps_realloc(void *ptr, std::size_t size, std::uint32_t caps) {
    ...
    if (resized != nullptr) {
        if (existing != fake_heap_caps_allocations.end()) {
            fake_heap_caps_allocations.erase(existing);
        }
...
inline std::size_t heap_caps_get_total_size(std::uint32_t caps) {
    if (fake_heap_caps_is_external(caps)) {
        return fake_heap_caps_external_total;
    }
    if ((caps & MALLOC_CAP_INTERNAL) != 0) {
        return fake_heap_caps_internal_total;
    }
    return fake_heap_caps_internal_total + fake_heap_caps_external_total;
}

inline std::size_t heap_caps_get_free_size(std::uint32_t caps) {
    if (fake_heap_caps_is_external(caps)) {
        return fake_heap_caps_external_free;
    }
    if ((caps & MALLOC_CAP_INTERNAL) != 0) {
        return fake_heap_caps_internal_free;
    }
    return fake_heap_caps_internal_free + fake_heap_caps_external_free;
}

inline std::size_t heap_caps_get_minimum_free_size(std::uint32_t caps) {
    if (fake_heap_caps_is_external(caps)) {
        return fake_heap_caps_external_minimum_free;
    }
    if ((caps & MALLOC_CAP_INTERNAL) != 0) {
        return fake_heap_caps_internal_minimum_free;
    }
    return fake_heap_caps_internal_minimum_free + fake_heap_caps_external_minimum_free;
}

inline std::size_t heap_caps_get_largest_free_block(std::uint32_t caps) {
    if (fake_heap_caps_is_external(caps)) {
        return fake_heap_caps_external_largest;
    }
    if ((caps & MALLOC_CAP_INTERNAL) != 0) {
        return fake_heap_caps_internal_largest;
    }
    return fake_heap_caps_internal_largest > fake_heap_caps_external_largest
        ? fake_heap_caps_internal_largest
        : fake_heap_caps_external_largest;
}
